...
function myAlphabetLength() {
    ...
    if (myAlphabet.length < 5) {
        compareTo5 = "Array length is less then 5.";
    } else {
        compareTo5 = "Array length is greater than or equal to 5.";
    }
    console.log("The length of myAlphabet: " + myAlphabet.length);
    console.log(compareTo5);
}
myAlphabetLength();
...
planets.forEach((planet, ind) =>
    console.log(`Planet: ${planet},  Index: ${ind}`)
)
...
for (let i = 0; i < wowDatatypes.length; i++) {
    console.log(
        `Item:  ${wowDatatypes[i]}, Index: ${wowDatatypes.indexOf(wowDatatypes[i])}, Type: ${typeof wowDatatypes[i]}`
    )
}
...
*/
// Write your code here
let commonSubjects = student1Courses.filter(course => student2Courses.includes(course));
commonSubjects.forEach(course =>
...
const filtered = temperatures.filter((item) => item > 0);
...
function getOddYears(arr) {
    return arr.filter(function (year) {
        return year % 2 !== 0
    });
}
...
//arrow function
const getOddYears = (arr) => arr.filter((year) => year % 2 !== 0);
console.log(getOddYears([2019, 2020, 2021]));
console.log(getOddYears([2000, 2015, 2018, 2020]));
