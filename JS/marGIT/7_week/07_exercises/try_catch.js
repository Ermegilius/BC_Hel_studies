...
function task2() {
  ...
  try {
    ...
    if (error instanceof ReferenceError) {
      console.log('ReferenceError error: ', error.message);
    } else {
...
function task5(array, index) {
  ...
  try {
    if (index < 0 || index >= array.length) {
      throw new RangeError("The index doesn't exist in the array (out of bounds)");
    } else {
      return array[index];
    }
  } catch (error) {
...
task6({ firstName: 'Vova' }, 'firstName');
...
function task7(jsonString) {
  ...
  try {
    ...
    if (typeof (jsonString)) {
      console.error('Caught an error, invalid JSON format: ', error.message);
    }
  } finally {
    console.log('The function was executed');
  }
